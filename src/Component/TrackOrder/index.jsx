...
export default function OrderTracker({ service, service_type_id }) {
  ...
  const serviceTypeIndex =
    service.service_types?.findIndex((st) => st.id === service_type_id) ?? 0;
  const priceData = calculateServicePrice(
    service,
    serviceTypeIndex >= 0 ? serviceTypeIndex : 0,
  );
                  ...
                  {step.options.map((option) => (
                    <Grid item xs={12} sm={6} key={option}>
                      <FormControlLabel
                        control={
                          <Checkbox
                            checked={(formData[step.title] || []).includes(
                              option,
                            )}
                            onChange={() =>
                              handleCheckboxChange(step.title, option)
                            }
                          />
                        }
                        label={option}
      ...
      <Box maxWidth={1000} mx="auto">
        ...
        <Box textAlign="center" mb={4}>
          ...
          <Typography color="text.secondary" fontSize={18}>
            {priceData.serviceType?.service_type_name ||
        ...
        <Paper
          elevation={1}
          sx={{ p: { xs: 3, md: 5 }, borderRadius: 3, mb: 3 }}
        >
          {renderStepContent()}
        </Paper>
