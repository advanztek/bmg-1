...
const ImagePreviewModal = ({ selectedImage, handleCloseModal, handleDeleteImage }) => {
    ...
    const imageUrl = selectedImage
        ? `${BASE_IMAGE_URL}/${selectedImage.image}`
        : "";
    ...
    const handleSocialShare = (platform) => {
        const text = selectedImage?.prompt || "Check out this AI-generated image!";
    ...
    const handleDownload = async () => {
        try {
            ...
            const response = await axios.get(
                `${BASE_SERVER_URL}/ai/download-image?url=${encodeURIComponent(imageUrl)}`,
                {
                    ...config,
                    responseType: 'blob',
                    onDownloadProgress: (progressEvent) => {
                        ...
                        // Update snackbar message with progress
                        setSnackbar({
                            open: true,
            ...
            // With axios, the blob is in response.data
            const blob = response.data;
            const url = window.URL.createObjectURL(blob);
            const link = document.createElement("a");
            link.href = url;
            link.download = `ai-generated-${selectedImage.id}-${Date.now()}.png`;
    ...
    const handleNativeShare = async () => {
        if (navigator.share) {
            try {
                ...
                const file = new File([blob], `image-${selectedImage.id}.png`, { type: "image/png" });
        ...
        <>
            ...
            >
                <DialogContent sx={{ p: 0, position: "relative" }}>
                    ...
                    {selectedImage && (
                        <Zoom in={Boolean(selectedImage)} timeout={300}>
                            <Box sx={{ position: "relative" }}>
                                ...
                                >
                                    ...
                                    {selectedImage.created_at && (
                                        <Box mb={2}>
                                            <Chip
                                                label={new Date(selectedImage.created_at).toLocaleDateString("en-US", {
                                                    month: "short",
                                                    day: "numeric",
                                                    year: "numeric",
                                                    hour: "2-digit",
